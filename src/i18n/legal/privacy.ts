...
export function privacySections(locale: Locale): { h2?: string; h3?: string; html: string }[] {
  if (locale === 'ja') {
    return [
      ...
      {
        h3: '2.5 ??????digest ???',
        html: `<p>??????????????<strong>???????</strong>????ja / en?? Supabase ??????????????????????????????? digest ???? Resend ??????????????????? <a href="${contactUrl}">GitHub Issues</a> ??????????</p>`,
      },
      {
        h2: '3. ??????????',
  ...
  return [
    ...
    {
      h3: '2.5 Email subscribe (digest notifications)',
      html: `<p>The footer subscribe form stores your <strong>email address</strong> and locale (ja / en) in Supabase after you opt in. We send weekday digest notifications via Resend. To unsubscribe, contact us via <a href="${contactUrl}">GitHub Issues</a>.</p>`,
    },
    {
      h2: '3. External links',
